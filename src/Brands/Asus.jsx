        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 mx-20">
            {
                ...
                <div key={product._id} className="bg-slate-200 p-10 rounded-md">
                    ...
                    <Link to={`/detailsAsus/${product._id}`}><button className="btn btn-accent mr-2">Details</button></Link>
                    <Link to={`/updateAsus/${product._id}`}><button className="btn btn-accent">Update</button></Link>
                </div>)
            }
        </div>
    );
